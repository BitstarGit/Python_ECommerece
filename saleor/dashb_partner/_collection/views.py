...
def collection_list(request):
    ...
    if request.method == "POST" and assign_homepage_col_form.is_valid():
        ...
        msg = pgettext_lazy("Dashboard message", "Updated homepage collection")
    ...
    collections = get_paginator_items(
        collection_filter.qs, settings.DASHBOARD_PAGINATE_BY, request.GET.get("page")
    )
    ctx = {
        "collections": collections,
        "filter_set": collection_filter,
        "is_empty": not collection_filter.queryset.exists(),
        "assign_homepage_col_form": assign_homepage_col_form,
    }
    return TemplateResponse(request, "dashboard/collection/list.html", ctx)
...
def collection_update(request, pk=None):
    ...
    is_unpublish_restricted = (
        collection == request.site.settings.homepage_collection
...
def collection_toggle_is_published(request, pk):
    ...
    return JsonResponse({"success": True, "is_published": collection.is_published})
