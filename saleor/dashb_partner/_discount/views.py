...
def sale_delete(request, pk):
    ...
    if request.method == "POST":
        instance.delete()
        msg = pgettext_lazy("Sale (discount) message", "Removed sale %s") % (
            instance.name,
        )
        messages.success(request, msg)
...
def voucher_list(request):
    vouchers = Voucher.objects.prefetch_related("products", "collections").order_by(
        "name"
    )
    voucher_filter = VoucherFilter(request.GET, queryset=vouchers)
    vouchers = get_paginator_items(
        voucher_filter.qs, settings.DASHBOARD_PAGINATE_BY, request.GET.get("page")
    )
    ctx = {
        "vouchers": vouchers,
        "filter_set": voucher_filter,
        "is_empty": not voucher_filter.queryset.exists(),
    }
    return TemplateResponse(request, "dashboard/discount/voucher/list.html", ctx)
...
def voucher_add(request):
    ...
    return TemplateResponse(request, "dashboard/discount/voucher/form.html", ctx)
...
def voucher_delete(request, pk):
    ...
    if request.method == "POST":
        instance.delete()
        msg = pgettext_lazy("Voucher message", "Removed voucher %s") % (instance,)
        messages.success(request, msg)
        return redirect("dashboard:voucher-list")
