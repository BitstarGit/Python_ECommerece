...
def shipping_zone_list(request):
    zones = ShippingZone.objects.prefetch_related("shipping_methods").order_by("name")
    shipping_zone_filter = ShippingZoneFilter(request.GET, queryset=zones)

    form = ChangeDefaultWeightUnit(request.POST or None, instance=request.site.settings)
    if request.method == "POST" and form.is_valid():
        form.save()
        msg = pgettext_lazy("Dashboard message", "Updated default weight unit")
        messages.success(request, msg)
        return redirect("dashboard:shipping-zone-list")

    zones = get_paginator_items(
        shipping_zone_filter.qs.distinct(),
        settings.DASHBOARD_PAGINATE_BY,
        request.GET.get("page"),
    )
    ctx = {
        "shipping_zones": zones,
        "filter_set": shipping_zone_filter,
...
def shipping_zone_delete(request, pk):
    ...
    if request.method == "POST":
        ...
        msg = pgettext_lazy(
            "Dashboard message", "%(shipping_zone_name)s successfully removed"
...
def shipping_method_add(request, shipping_zone_pk, type):
    ...
    shipping_method = ShippingMethod(shipping_zone_id=shipping_zone_pk, type=type)
    ...
    if form.is_valid():
        ...
        messages.success(request, msg)
        return redirect("dashboard:shipping-zone-details", pk=shipping_zone_pk)
    ctx = {
        "form": form,
        "shipping_zone": shipping_zone,
        "shipping_method": shipping_method,
    }
    return TemplateResponse(request, "dashboard/shipping/methods/form.html", ctx)
...
def shipping_method_edit(request, shipping_zone_pk, shipping_method_pk):
    ...
    if form.is_valid():
        ...
        msg = pgettext_lazy(
            "Dashboard message", "Updated %(method_name)s shipping method"
...
@partner_member_required
@permission_required("shipping.manage_shipping")
def shipping_method_delete(request, shipping_zone_pk, shipping_method_pk=None):
    shipping_method = get_object_or_404(ShippingMethod, pk=shipping_method_pk)
    if request.method == "POST":
        shipping_method.delete()
        msg = pgettext_lazy(
            "Dashboard message", "Removed %(shipping_method_name)s shipping method"
        ) % {"shipping_method_name": shipping_method}
        messages.success(request, msg)
        return redirect("dashboard:shipping-zone-details", pk=shipping_zone_pk)
    ctx = {"shipping_method": shipping_method, "shipping_zone_pk": shipping_zone_pk}
    return TemplateResponse(
