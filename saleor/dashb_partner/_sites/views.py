...
def site_settings_edit(request, pk):
    ...
    if site_settings_form.is_valid() and site_form.is_valid():
        ...
        messages.success(
            request, pgettext_lazy("Dashboard message", "Updated site settings")
        )
...
def site_settings_details(request, pk):
    site_settings = get_object_or_404(SiteSettings, pk=pk)
    authorization_keys = AuthorizationKey.objects.filter(site_settings=site_settings)
    ctx = {
        "site_settings": site_settings,
...
def authorization_key_add(request, site_settings_pk):
    ...
    if form.is_valid():
        key = form.save()
        msg = pgettext_lazy("Dashboard message", "Added authorization key %s") % (key,)
...
def authorization_key_edit(request, site_settings_pk, key_pk):
    ...
    if form.is_valid():
        key = form.save()
        msg = pgettext_lazy("dashboard message", "Updated authorization key %s") % (
            key,
        )
        messages.success(request, msg)
        return redirect("dashboard:site-details", pk=site_settings_pk)
...
def authorization_key_delete(request, site_settings_pk, key_pk):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            pgettext_lazy("Dashboard message", "Removed site authorization key %s")
            % (key,),
        )
        return redirect("dashboard:site-details", pk=site_settings_pk)
    return TemplateResponse(
        request,
        "dashboard/sites/modal/confirm_delete.html",
