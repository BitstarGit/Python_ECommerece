...
def staff_list(request):
    ...
    staff_members = get_paginator_items(
        staff_filter.qs, settings.DASHBOARD_PAGINATE_BY, request.GET.get("page")
    )
    ctx = {
        "staff": staff_members,
...
def staff_details(request, pk):
    ...
    form = StaffForm(request.POST or None, instance=staff_member, user=request.user)
    if form.is_valid():
        form.save()
        msg = pgettext_lazy("Dashboard message", "Updated staff member %s") % (
            staff_member,
        )
...
def staff_delete(request, pk):
    ...
    if request.method == "POST":
        ...
        msg = pgettext_lazy("Dashboard message", "Removed staff member %s") % (staff,)
        messages.success(request, msg)
        return redirect("dashboard:staff-list")
    ctx = {"staff": staff, "orders": staff.orders.count()}
    return TemplateResponse(request, "dashboard/staff/modal/confirm_delete.html", ctx)
