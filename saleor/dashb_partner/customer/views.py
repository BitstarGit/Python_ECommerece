...
from ...partner.utils import create_partner_customer
...
def customer_list(request):
    customers = (
        User.objects.filter(
            ...
            partner_customer__partner=request.user.partner
        )
        .distinct()
    ...
    customers = get_paginator_items(
        customer_filter.qs, settings.DASHBOARD_PAGINATE_BY, request.GET.get("page")
    )
    ctx = {
        "customers": customers,
        "filter_set": customer_filter,
        "is_empty": not customer_filter.queryset.exists(),
    }
    return TemplateResponse(request, "dashb_partner/customer/list.html", ctx)
...
def customer_details(request, pk):
    ...
    ctx = {"customer": customer, "customer_orders": customer_orders, "notes": notes}
    return TemplateResponse(request, "dashb_partner/customer/detail.html", ctx)
...
def customer_create(request):
    ...
    if form.is_valid():
        ...
        create_partner_customer(request.user, customer)

        send_set_password_email(customer)
        messages.success(request, msg)
        return redirect("partner:customer-details", pk=customer.pk)
    ctx = {"form": form, "customer": customer}
    return TemplateResponse(request, "dashb_partner/customer/form.html", ctx)
...
def ajax_users_list(request):
    ...
    users = [{"id": user.pk, "text": user.get_ajax_label()} for user in queryset]
...
def customer_add_note(request, customer_pk):
    ...
    if form.is_valid():
        form.save()
        msg = pgettext_lazy("Dashboard message related to an customer", "Added note")
        messages.success(request, msg)
    elif form.errors:
        status = 400
    ctx = {"customer": customer, "form": form}
    ctx.update(csrf(request))
    template = "dashb_partner/customer/modal/add_note.html"
...
def customer_delete(request, pk):
    ...
    return TemplateResponse(
        request, "dashb_partner/customer/modal/confirm_delete.html", ctx, status=status
    )
