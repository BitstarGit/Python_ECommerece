...
def send_set_password_email_with_url(redirect_url, user, staff=False):
    ...
    template = f"dashb_partner/{template_type}/set_password"
...
def send_promote_customer_to_staff_email(staff_pk):
    ...
    send_kwargs, ctx = get_email_context()
    ctx["dashboard_url"] = build_absolute_uri(reverse("partner:index"))
    send_templated_mail(
        template_name="dashb_partner/staff/promote_customer",
