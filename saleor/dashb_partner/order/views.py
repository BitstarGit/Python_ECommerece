...
from ...order.emails import send_fulfillment_confirmation_to_customer, send_payment_confirmation
from ...partner.utils import create_partner_order
...
from ...partner.models import PartnerFulfillment, PartnerFulfillmentLine
from ...order.utils import update_order_prices
from ...shipping.models import ShippingMethod
from ...partner.models import PartnerOrder, Partner, PartnerFulfillment
from ..views import partner_member_required
...
def order_list(request):
    orders = Order.objects.filter(orders_partner__partner=request.user.partner, orders_partner__visible=True)\
        .select_related("user").prefetch_related("payments", "lines", "user")
    ...
    return TemplateResponse(request, "dashb_partner/order/list.html", ctx)
...
def order_create(request):
    ...
    create_partner_order(request.user, order)

    # Send success message and redirect to the draft details
    messages.success(request, msg)
    return redirect("partner:order-details", order_pk=order.pk)
...
def create_order_from_draft(request, order_pk):
    ...
    if form.is_valid():
        ...
        status = 400
    template = "dashb_partner/order/modal/create_order.html"
...
def remove_draft_order(request, order_pk):
    ...
    template = "dashb_partner/order/modal/remove_order.html"
    ctx = {"order": order}
...
def order_details(request, order_pk):
    qs = Order.objects.select_related(
        ...
        "fulfillments__partnerfulfillments",
        "fulfillments__lines__partner_lines",
    )
    order = get_object_or_404(qs, pk=order_pk)
    
    theOrder = Order.objects.get(id=order.id)
    ...
    return TemplateResponse(request, "dashb_partner/order/detail.html", ctx)
...
def order_add_note(request, order_pk):
    ...
    template = "dashb_partner/order/modal/add_note.html"
    return TemplateResponse(request, template, ctx, status=status)
...
def capture_payment(request, order_pk, payment_pk):
    ...
    return TemplateResponse(
        request, "dashb_partner/order/modal/capture.html", ctx, status=status
    )
...
def refund_payment(request, order_pk, payment_pk):
    ...
    return TemplateResponse(
        request, "dashb_partner/order/modal/refund.html", ctx, status=status
    )
...
def void_payment(request, order_pk, payment_pk):
    ...
    return TemplateResponse(
        request, "dashb_partner/order/modal/void.html", ctx, status=status
    )
...
def orderline_change_quantity(request, order_pk, line_pk):
    ...
    template = "dashb_partner/order/modal/change_quantity.html"
    return TemplateResponse(request, template, ctx, status=status)
...
def orderline_cancel(request, order_pk, line_pk):
    ...
    return TemplateResponse(
        request, "dashb_partner/order/modal/cancel_line.html", ctx, status=status
    )
...
def add_variant_to_order(request, order_pk):
    ...
    if form.is_valid():
        ...
        return redirect("partner:order-details", order_pk=order_pk)
    elif form.errors:
        status = 400
    ctx = {"order": order, "form": form}
    template = "dashb_partner/order/modal/add_variant_to_order.html"
    return TemplateResponse(request, template, ctx, status=status)
...
def order_address(request, order_pk, address_type):
    ...
    return TemplateResponse(request, "dashb_partner/order/address_form.html", ctx)
...
def order_customer_edit(request, order_pk):
    ...
    ctx = {"order": order, "form": form}
    return TemplateResponse(
        request, "dashb_partner/order/modal/edit_customer.html", ctx, status=status
    )
...
def order_customer_remove(request, order_pk):
    ...
    if form.is_valid():
        ...
        messages.success(request, msg)
        return redirect("partner:order-details", order_pk=order_pk)
    return redirect("partner:order-customer-edit", order_pk=order.pk)
...
def order_shipping_edit(request, order_pk):
    ...
    ctx = {"order": order, "form": form}
    return TemplateResponse(
        request, "dashb_partner/order/modal/edit_shipping.html", ctx, status=status
    )
...
def order_shipping_remove(request, order_pk):
    ...
    if form.is_valid():
        ...
        messages.success(request, msg)
        return redirect("partner:order-details", order_pk=order_pk)
    return redirect("partner:order-shipping-edit", order_pk=order.pk)
...
def order_discount_edit(request, order_pk):
    ...
    ctx = {"order": order, "form": form}
    return TemplateResponse(
        request, "dashb_partner/order/modal/edit_discount.html", ctx, status=status
    )
...
def order_voucher_edit(request, order_pk):
    ...
    ctx = {"order": order, "form": form}
    return TemplateResponse(
        request, "dashb_partner/order/modal/edit_voucher.html", ctx, status=status
    )
...
def cancel_order(request, order_pk):
    ...
    ctx = {"form": form, "order": order}
    return TemplateResponse(
        request, "dashb_partner/order/modal/cancel_order.html", ctx, status=status
    )

@partner_member_required
def cancel_order1(request, order_pk):
    orders = Order.objects.confirmed().prefetch_related("lines")
    order = get_object_or_404(orders, pk=order_pk)
    ...
    if form.is_valid():
        ...
        with transaction.atomic():
            form.cancel_order(request.user)
        # messages.success(request, msg)
        return redirect("/partner/orders/list/")
    ...
    ctx = {"form": form, "order": order}
    return TemplateResponse(
        request, "dashb_partner/order/modal/cancel_order1.html", ctx, status=status
    )
...
def order_voucher_remove(request, order_pk):
    ...
    if form.is_valid():
        ...
        with transaction.atomic():
            ...
            messages.success(request, msg)
        return redirect("partner:order-details", order_pk=order.pk)
    return redirect("partner:order-voucher-edit", order_pk=order.pk)
...
def mark_order_as_paid(request, order_pk):
    ...
    ctx = {"form": form, "order": order}
    return TemplateResponse(
        request, "dashb_partner/order/modal/mark_as_paid.html", ctx, status=status
    )
...
def fulfill_order_lines(request, order_pk):
    ...
    if all_line_forms_valid and formset.is_valid() and form.is_valid():
        ...
        if forms_to_save:
            ...
            for line_form in forms_to_save:
                ...
                linestring = "sid_form-"+str(index)+"-quantity"
                line.line_status = request.POST[linestring]
                line.save()
                
                obj1, created = PartnerFulfillmentLine.objects.get_or_create(fulfillment_line=FulfillmentLine.objects.get(id=line.id))
                obj1.line_status = request.POST[linestring]
                pricestring = "uid_form-"+str(index)+"-quantity"
                obj1.unit_price = request.POST[pricestring]
                obj1.save()
            ...
            theOrder = Order.objects.get(id=order.id)
            obj, created = PartnerFulfillment.objects.get_or_create(fulfillment=Fulfillment.objects.get(id=fulfillment.id), partner=Partner.objects.get(user=request.user))
            obj.est_shipping_date=request.POST['estimated_shipping_date']
            obj.partner_status=request.POST['select_type']
            obj.amount = request.POST['amount_price']
            obj.save()

            if theOrder.status == "fulfilled":
                for fulfill in order.fulfillments.all():
                    PartnerFulfillment.objects.filter(fulfillment_id=fulfill.id).update(partner_status="1")

                current_site = get_current_site(request)
                mail_subject = 'Hi'
                message = render_to_string('confirmation_email.html', {
                    'user':request.user, 'domain':current_site.domain,"order_fulfillments": order.fulfillments.all(),
                })
    ...
    template = "dashb_partner/order/fulfillment_form.html"
    return TemplateResponse(request, template, ctx, status=status)
...
def cancel_fulfillment(request, order_pk, fulfillment_pk):
    ...
    return TemplateResponse(
        request, "dashb_partner/order/modal/cancel_fulfillment.html", ctx, status=status
    )
...
def change_fulfillment_tracking(request, order_pk, fulfillment_pk):
    ...
    ctx = {"form": form, "order": order, "fulfillment": fulfillment}
    return TemplateResponse(
        request, "dashb_partner/order/modal/fulfillment_tracking.html", ctx, status=status
    )

@partner_member_required
def change_fulfillment_status(request, order_pk, fulfillment_pk):
    orders = Order.objects.confirmed().prefetch_related("fulfillments__partnerfulfillments")
    order = get_object_or_404(orders, pk=order_pk)
    fulfillment = get_object_or_404(order.fulfillments, pk=fulfillment_pk)
    status = 200
    form = FulfillmentTrackingNumberForm(request.POST or None, instance=fulfillment)
    if form.is_valid():
        PartnerFulfillment.objects.filter(fulfillment=Fulfillment.objects.get(id=fulfillment.id)).update(partner_status=request.POST['select_type_update'])
        msg = pgettext_lazy(
            "Dashboard message", "Fulfillment #%(fulfillment)s Status updated"
        ) % {"fulfillment": fulfillment.composed_id}
        messages.success(request, msg)
        return redirect("partner:order-details", order_pk=order.pk)
    elif form.errors:
    ...
    ctx = {"form": form, "order": order, "fulfillment": fulfillment}
    return TemplateResponse(
        request, "dashb_partner/order/modal/fulfillment_status.html", ctx, status=status
    )

@partner_member_required
def change_fulfillment_date(request, order_pk, fulfillment_pk):
    orders = Order.objects.confirmed().prefetch_related("fulfillments__partnerfulfillments")
    order = get_object_or_404(orders, pk=order_pk)
    fulfillment = get_object_or_404(order.fulfillments, pk=fulfillment_pk)
    status = 200
    form = FulfillmentTrackingNumberForm(request.POST or None, instance=fulfillment)
    if form.is_valid():
        PartnerFulfillment.objects.filter(fulfillment=Fulfillment.objects.get(id=fulfillment.id)).update(est_shipping_date=request.POST['est_shipping_date_update'])
        msg = pgettext_lazy(
            "Dashboard message", "Fulfillment #%(fulfillment)s tracking number updated"
        ) % {"fulfillment": fulfillment.composed_id}
        messages.success(request, msg)
    ...
    ctx = {"form": form, "order": order, "fulfillment": fulfillment}
    return TemplateResponse(
        request, "dashb_partner/order/modal/fulfillment_date.html", ctx, status=status
    )
