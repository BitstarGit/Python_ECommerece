...
from ...partner.utils import update_partner_variant, create_partner_product
...
def product_list(request):
    products = Product.objects\
        .filter(Q(partner_products__partner=request.user.partner) | Q(is_standard=True)).prefetch_related("images")

    products = products.order_by("is_standard", "name")
    ...
    return TemplateResponse(request, "dashb_partner/product/list.html", ctx)
...
def product_details(request, pk):
    ...
    return TemplateResponse(request, "dashb_partner/product/detail.html", ctx)
...
def product_select_type(request):
    ...
    if form.is_valid():
        ...
        status = 400
    ctx = {"form": form}
    template = "dashb_partner/product/modal/select_type.html"
...
def product_create(request, type_pk):
    ...
    if product_form.is_valid() and not variant_errors:
        ...
        create_partner_product(request.user, product)
        ...
        return redirect("partner:product-details", pk=product.pk)
    ctx = {
        "product_form": product_form,
        "variant_form": variant_form,
        "product": product,
    }
    return TemplateResponse(request, "dashb_partner/product/form.html", ctx)
...
def product_delete(request, pk):
    ...
    if request.method == "POST":
        product.delete()
        msg = pgettext_lazy("Dashboard message", "Removed product %s") % (product,)
        messages.success(request, msg)
        return redirect("partner:product-list")
    return TemplateResponse(
        request, "dashb_partner/product/modal/confirm_delete.html", {"product": product}
    )
...
def ajax_products_list(request):
    ...
    products = [{"id": product.id, "text": str(product)} for product in queryset]
...
def product_type_list(request):
    types = (
        ...
        .prefetch_related("product_attributes", "variant_attributes")
        .order_by("name")
    )
    type_filter = ProductTypeFilter(request.GET, queryset=types)
    types = get_paginator_items(
        type_filter.qs, settings.DASHBOARD_PAGINATE_BY, request.GET.get("page")
    )
    types.object_list = [
        (pt.pk, pt.name, pt.product_attributes.all(), pt.variant_attributes.all())
        for pt in types.object_list
    ]
    ctx = {
        "product_types": types,
        "filter_set": type_filter,
        "is_empty": not type_filter.queryset.exists(),
    }
    return TemplateResponse(request, "dashb_partner/product/product_type/list.html", ctx)
...
def product_type_create(request):
    ...
    if form.is_valid():
        ...
        msg = pgettext_lazy("Dashboard message", "Added product type %s") % (
            product_type,
        )
        messages.success(request, msg)
        return redirect("partner:product-type-list")
    ctx = {"form": form, "product_type": product_type}
    return TemplateResponse(request, "dashb_partner/product/product_type/form.html", ctx)
...
def product_type_edit(request, pk):
    ...
    if form.is_valid():
        product_type = form.save()
        msg = pgettext_lazy("Dashboard message", "Updated product type %s") % (
            product_type,
        )
        messages.success(request, msg)
        return redirect("partner:product-type-update", pk=pk)
    ctx = {"form": form, "product_type": product_type}
    return TemplateResponse(request, "dashb_partner/product/product_type/form.html", ctx)
...
def product_type_delete(request, pk):
    ...
    if request.method == "POST":
        product_type.delete()
        msg = pgettext_lazy("Dashboard message", "Removed product type %s") % (
            product_type,
        )
        messages.success(request, msg)
        return redirect("partner:product-type-list")
    ctx = {"product_type": product_type, "products": product_type.products.all()}
    return TemplateResponse(
        request, "dashb_partner/product/product_type/modal/confirm_delete.html", ctx
    )
...
def variant_details(request, product_pk, variant_pk):
    ...
    return TemplateResponse(
        request, "dashb_partner/product/product_variant/detail.html", ctx
    )
...
def variant_create(request, product_pk):
    ...
    if form.is_valid():
        ...
        messages.success(request, msg)
        return redirect(
            "partner:variant-details", product_pk=product.pk, variant_pk=variant.pk
        )
    ctx = {"form": form, "product": product, "variant": variant}
    return TemplateResponse(request, "dashb_partner/product/product_variant/form.html", ctx)
...
def variant_edit(request, product_pk, variant_pk):
    ...
    update_partner_variant(request.user, product, variant)
...
def variant_delete(request, product_pk, variant_pk):
    ...
    return TemplateResponse(
        request, "dashb_partner/product/product_variant/modal/confirm_delete.html", ctx
    )
...
def variant_images(request, product_pk, variant_pk):
    ...
    ctx = {"form": form, "product": product, "variant": variant}
    return TemplateResponse(
        request, "dashb_partner/product/product_variant/modal/select_images.html", ctx
    )
...
def ajax_available_variants_list(request):
    ...
    available_products = Product.objects.published().prefetch_related(
        "category", "product_type__product_attributes"
    )
    queryset = ProductVariant.objects.filter(
        product__in=available_products
    ).prefetch_related("product__category", "product__product_type__product_attributes")
...
def product_images(request, product_pk):
    ...
    images = product.images.all()
    ctx = {"product": product, "images": images, "is_empty": not images.exists()}
    return TemplateResponse(request, "dashb_partner/product/product_image/list.html", ctx)
...
def product_image_create(request, product_pk):
    ...
    if form.is_valid():
        ...
        messages.success(request, msg)
        return redirect("partner:product-image-list", product_pk=product.pk)
    ctx = {"form": form, "product": product, "product_image": product_image}
    return TemplateResponse(request, "dashb_partner/product/product_image/form.html", ctx)
...
def product_image_edit(request, product_pk, img_pk):
    ...
    if form.is_valid():
        product_image = form.save()
        msg = pgettext_lazy("Dashboard message", "Updated image %s") % (
            product_image.image.name,
        )
        messages.success(request, msg)
        return redirect("partner:product-image-list", product_pk=product.pk)
...
def product_image_delete(request, product_pk, img_pk):
    ...
    return TemplateResponse(
        request,
        "dashb_partner/product/product_image/modal/confirm_delete.html",
...
def attribute_list(request):
    ...
    return TemplateResponse(request, "dashb_partner/product/attribute/list.html", ctx)
...
def attribute_details(request, pk):
    ...
    return TemplateResponse(request, "dashb_partner/product/attribute/detail.html", ctx)
...
def attribute_create(request):
    ...
    if form.is_valid():
        ...
        messages.success(request, msg)
        return redirect("partner:attribute-details", pk=attribute.pk)
    ctx = {"attribute": attribute, "form": form}
    return TemplateResponse(request, "dashb_partner/product/attribute/form.html", ctx)
...
def attribute_delete(request, pk):
    ...
    if request.method == "POST":
        attribute.delete()
        msg = pgettext_lazy("Dashboard message", "Removed attribute %s") % (
            attribute.name,
        )
        messages.success(request, msg)
        return redirect("partner:attributes")
    return TemplateResponse(
        request,
        "dashb_partner/product/attribute/modal/" "attribute_confirm_delete.html",
        {"attribute": attribute},
    )
...
def attribute_value_create(request, attribute_pk):
    ...
    if form.is_valid():
        form.save()
        msg = pgettext_lazy("Dashboard message", "Added attribute's value")
        messages.success(request, msg)
        return redirect("partner:attribute-details", pk=attribute_pk)
    ctx = {"attribute": attribute, "value": value, "form": form}
    return TemplateResponse(
        request, "dashb_partner/product/attribute/values/form.html", ctx
    )
...
def attribute_value_edit(request, attribute_pk, value_pk):
    ...
    if form.is_valid():
        form.save()
        msg = pgettext_lazy("Dashboard message", "Updated attribute's value")
        messages.success(request, msg)
        return redirect("partner:attribute-details", pk=attribute_pk)
    ctx = {"attribute": attribute, "value": value, "form": form}
    return TemplateResponse(
...
def attribute_value_delete(request, attribute_pk, value_pk):
    ...
    if request.method == "POST":
        value.delete()
        msg = pgettext_lazy("Dashboard message", "Removed attribute's value %s") % (
            value.name,
        )
        messages.success(request, msg)
        return redirect("partner:attribute-details", pk=attribute_pk)
    return TemplateResponse(
        request,
        "dashb_partner/product/attribute/values/modal/confirm_delete.html",
...
def ajax_reorder_attribute_values(request, attribute_pk):
    attribute = get_object_or_404(Attribute, pk=attribute_pk)
    form = forms.ReorderAttributeValuesForm(request.POST, instance=attribute)
    status = 200
    ctx = {}
    if form.is_valid():
        form.save()
