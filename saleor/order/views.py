...
from ..dashboard.order.forms import CancelOrderForm
...
from .models import Order,FulfillmentLine, OrderLine
from .utils import attach_order_to_user, check_order_status
from ..partner.utils import handle_partner_checkout_success
...
def details(request, token):
    ...
    if request.method == "POST":
        if request.POST.get('order_status_decide') == "1":
            Order.objects.filter(id=order.id).update(new_order_status="fulfilled")
            for fulfillmentlins in fulfillments:
                deniedfullines = FulfillmentLine.objects.filter(fulfillment_id = fulfillmentlins.id)
                for deniedfulline in deniedfullines:
                    for partner_line in deniedfulline.partner_lines.all():
                        if partner_line.line_status == 'Deny':
                            default_amount = OrderLine.objects.get(id=deniedfulline.order_line_id).quantity
                            updated_amount = default_amount - deniedfulline.quantity
                            unit_price_amount = OrderLine.objects.get(id=deniedfulline.order_line_id).unit_price_net_amount
                            updated_price_amount = Order.objects.get(id=order.id).total_gross_amount - unit_price_amount * deniedfulline.quantity
                            updated_price_amount1 = Order.objects.get(id=order.id).total_net_amount - unit_price_amount * deniedfulline.quantity
                            OrderLine.objects.filter(id=deniedfulline.order_line_id).update(quantity=updated_amount)
                            Order.objects.filter(id=order.id).update(total_gross_amount=updated_price_amount)
                            Order.objects.filter(id=order.id).update(total_net_amount=updated_price_amount1)
                            deniedfulline.delete()

            return redirect("order:details", token=order.token)
        elif request.POST.get('order_status_decide') == "2":
            Order.objects.filter(id=order.id).update(new_order_status="declined")
            return redirect("order:details", token=order.token)
    ...
    ctx = {"order": order,"new_order_status":theOrder.new_order_status, 'order_status':theOrder.status, "fulfillments": fulfillments, "note_form": note_form}
...
def payment(request, token):
    ...
    theOrder = Order.objects.get(id=order.id)
    newOrderstatus = ""
    if theOrder.new_order_status == "fulfilled":
        newOrderstatus = "Yes"
...
def checkout_success(request, token):
    ...
    handle_partner_checkout_success(request.user, order)
